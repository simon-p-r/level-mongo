...
module.exports = class Collection {
    ...
    findOne(query, cb) {
        ...
        if (!Utils.isString(key)) {
            return cb(new TypeError('key must be a string'), null);
        }
    ...
    insertOne(doc, cb) {
        ...
        if (!Utils.isObj(doc)) {
            return cb(new TypeError('document is not an object'), null);
        }
    ...
    insertMany(recs, cb) {
        ...
        if (!Utils.isArray(recs)) {
            return cb(new TypeError('batch insert must be an array of documents'), null);
        }
        ...
        this.getKeys((err, keys) => {
            ...
            const iterator = (rec, next) => {
                ...
                if (!Utils.isObj(rec)) {
                    return next(new TypeError('document is not an object'), null);
                }

                if (!Utils.isString(key)) {
                    return next(new TypeError('key must be a string'), null);
                }
